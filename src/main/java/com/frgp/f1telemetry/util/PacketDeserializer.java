...
import com.frgp.f1telemetry.entity.*;
...
/**
 * F1 2019 PacketDeserializer is the main class for deserializing the incoming
 ...
 *
 * @author frgp
 * @link https://forums.codemasters.com/topic/44592-f1-2019-udp-specification
 */
public class PacketDeserializer {
     ...
     *
     * @param data : a F1 2019 UDP packet
    ...
    private CarMotionData buildCarMotionData(int carIndex, boolean playersCar) {
        ...
        final float denormalize = 32767.0f;
        ...
        carMotionData.setWorldForwardDirX(buffer.getNextUInt16AsInt() / denormalize);
        carMotionData.setWorldForwardDirY(buffer.getNextUInt16AsInt() / denormalize);
        carMotionData.setWorldForwardDirZ(buffer.getNextUInt16AsInt() / denormalize);
        carMotionData.setWorldRightDirX(buffer.getNextUInt16AsInt() / denormalize);
        carMotionData.setWorldRightDirY(buffer.getNextUInt16AsInt() / denormalize);
        carMotionData.setWorldRightDirZ(buffer.getNextUInt16AsInt() / denormalize);
    ...
    private PacketEventData buildPacketEventData(Header header) {
        ...
        switch (eventData.getEventCode()) {
            case "FTLP":
                eventData.setEventDetailsVehicleIdx(buffer.getNextUInt8AsInt());
                eventData.setLapTime(buffer.getNextFloat());
                break;
            case "RTMT":
            case "TMPT":
            case "RCWN":
     ...
     * PacketHeader    m_header;            // Header
     *
     * uint8           m_numCars;           // Number of cars in the data – should match number of cars on HUD
    ...
    private ParticipantData buildParticipantData() {
        ...
        participant.setYourTelemetry(buffer.getNextUInt8AsInt());
        return participant;
    }
    ...
    private ButtonStatus buildButtonStatus() {
        ...
        controller.setCrossAPressed((flags & 1) == 1);
        controller.setTriangleYPressed((flags >> 1 & 1) == 1);
        controller.setCircleBPressed((flags >> 2 & 1) == 1);
        controller.setSquareXPressed((flags >> 3 & 1) == 1);
        controller.setDpadLeftPressed((flags >> 4 & 1) == 1);
        controller.setDpadRightPressed((flags >> 5 & 1) == 1);
        controller.setDpadUpPressed((flags >> 6 & 1) == 1);
        controller.setDpadDownPressed((flags >> 7 & 1) == 1);
        controller.setOptionsMenuPressed((flags >> 8 & 1) == 1);
        controller.setL1LBPressed((flags >> 9 & 1) == 1);
        controller.setR1RBPressed((flags >> 10 & 1) == 1);
        controller.setL2LTPressed((flags >> 11 & 1) == 1);
        controller.setR2RTPressed((flags >> 12 & 1) == 1);
        controller.setLeftStickPressed((flags >> 13 & 1) == 1);
        controller.setRightStickPressed((flags >> 14 & 1) == 1);
